...
namespace SqlExpressUtilities
{
    ...
    public class SqlExpressWrapper
    {
        ...
        public virtual bool VerifierConnection(string email, string password)
        {
            bool existe = false; 
            // instancier l'objet de collection
            using (connection = new SqlConnection(connexionString))
            {
                // bâtir l'objet de requête
                using (SqlCommand sqlcmd = new SqlCommand("SELECT * FROM USAGERS WHERE EMAIL ='" + email + "' AND PASSWORD = '"+ password + "'"))
                {
                    // affecter l'objet de connection à l'objet de requête
                    sqlcmd.Connection = connection;
                    // ouvrir la connection avec la bd
        ...
        public virtual string TrouverPassword(string email)
        {
            string password = "";
            // instancier l'objet de collection
            using (connection = new SqlConnection(connexionString))
            {
                // bâtir l'objet de requête
                using (SqlCommand sqlcmd = new SqlCommand("SELECT PASSWORD FROM USAGERS WHERE EMAIL ='" + email + "'"))
                {
                    // affecter l'objet de connection à l'objet de requête
                    sqlcmd.Connection = connection;
                    // ouvrir la connection avec la bd
